...
The code is available at \url{https://github.com/GalacticDynamics-Oxford/Agama}.
...
\hline
\ttt{None} & --- & --- \\
\ttt{Reflection} & \mbox{$\{x,y,z\} \to \{-x,-y,-z\}$} \mbox{(twofold discrete symmetry)} & odd $\ell$ \\
...
\ttt{Spherical} & \mbox{same or rotation about origin by any angle} \mbox{(continuous symmetry in both $\theta$ and $\phi$)} & same + any $\ell \ne 0$
\end{tabular}
\end{table}
...
\item \ppp{eta} [1] -- parameter controlling the shape of basis functions in the Zhao \ttt{BasisSet} \cite{Zhao1996}. The zeroth-order function is a double-power-law (\ttt{Spheroid}) profile with $\alpha=1/\eta$, $\beta=3+1/\eta$ and $\gamma=2-1/\eta$; the default value $\eta=1$ corresponds to the widely used Hernquist--Ostriker basis set \cite{HernquistOstriker1992}, although values up to 2 and even higher may provide more accurate results for cuspy models.
\item \ppp{r0} -- scale radius of basis functions in \ttt{BasisSet}. If not provided, it is set to the half-mass radius of the density profile (unless the latter has infinite mass, in which case one needs to specify \ppp{r0} explicitly), and this choice is close to optimal for the approximation accuracy.
\end{itemize}
...
The class \ttt{ActionMapperTorus} implements the torus construction ``on-the-fly'', with each unique triplet of actions producing a new torus, which is then cached and reused in subsequent calls that use the same $\bJ$ but different $\bt$. The torus code is adapted from the original \textsc{TorusMapper} package, with several modifications enabling the use of an arbitrary potential and a more efficient angle mapping approach; however, it does not quite comply to the coding standards adopted in \Agama (Section~\ref{sec:DeveloperGuide}). A new and improved implementation of the Torus mapping \cite{Binney2026} is provided in \textsc{Agama}b, which is a separate code that has considerably diverged from the main branch over time; this new implementation may be ported into the main branch in the future. \\
...
For some combinations of parameters, this produces a DF which is negative in some range of $Q$, in particular, when the central slopes of the density profile $\gamma \equiv -d\ln\rho/d\ln r$, the potential $\delta \equiv -d\ln(-\Phi)/d\ln r$, and the coefficient $\beta_0$ violate the so-called slope--anisotropy theorem \cite{AnEvans2006}: $\gamma \ge 2\beta + (1/2-\beta)\delta$. For the self-gravitating case, $\delta=0$ if the potential is finite at origin ($\gamma<2$), or $\delta=\gamma-2$ otherwise. If the computed DF is negative, it is replaced by zero. Note that the code does not trigger an error or even a warning in this case, because it may occur due to numerical inaccuracies even if the DF is expected to be physically valid. However, the density profile generated by such a DF, in which negative values have been trimmed to zero, will deviate from the input profile, even if only slightly.
...
using the parameters listed in Section~\ref{sec:PotentialFactory}; parameter names are case-insensitive, as are the names of density models. Note that the list of parameters relevant for a specific model depends on the \texttt{type} argument.
...
If \ppp{type="Multipole"}, \ppp{"BasisSet"} or \ppp{"CylSpline"}, then a potential expansion is constructed from the given density or potential profile in one of the following mutually exclusive ways.
...
\\[2mm]
A composite potential can be created from several other \ttt{Potential} objects:\\
...
\texttt{pot_list = list(map(agama.Potential, [disk_par, bulge_par, halo_par]))}\\
...
An \ppp{Evolving} potential containing several instances of other potentials can be read from an INI file or created manually by providing a \texttt{timestamps} argument (a dictionary with potentials indexed by timestamps):\\
\texttt{p67 = agama.Potential(type="Evolving", timestamps=\{0.0:~p6, 1.0:~p7\}\\
\mbox{}~~~~[, interpLinear=True])}\\
When evaluated at different moments of time, it will give different results (either picking up the nearest potential instance or linearly interpolating between two adjacent ones). It can be indexed, like a composite potential, but this returns only the underlying potentials instances without associated timestamps.\\
\texttt{assert numpy.allclose(p67.potential(point, t=0.25), \\
\mbox{}~~~~0.75 * p6.potential(point) + 0.25 * p7.potential(point))}\\
\texttt{assert p67[1] == p7}\\
When all elements of the \ppp{Evolving} potential are instances of \ppp{BasisSet} sharing the same order of expansion and scale radius, and linear time interpolation is requested, this produces a time-dependent \ppp{BasisSet} potential that performs the interpolation at the level of coefficients rather than entire potentials; this is only slightly more expensive than evaluating a time-independent \ppp{BasisSet}, whereas a generic \ppp{Evolving} potential is twice more expensive. A similar optimization for \ppp{Multipole} or \ppp{CylSpline} potentials would give only marginal savings, and is not implemented.
\\[2mm]
...
\paragraph{DistributionFunction} class provides the \Python interface to the hierarchy of \Cpp DF classes. It is constructed either from a list of keyword arguments (case-insensitive),\\[1mm]
...
\paragraph{SelectionFunction} class provides an example of a position-dependent selection function (SF) for the \ttt{GalaxyModel} class: $S(\bx, \bv) = \exp\big[-(|\bx-\bx_0|/r_\text{cut})^\xi\big] \big/ \big[1 + (|\bx-\bx_0|/r_0)^\gamma\big]$. Here $\bx_0$ is the fiducial point in configuration space, and other parameters control the decay of SF with distance from this point (two modes, which may be applied simultaneously): $r_\text{cut}$ is the radius of the exponential cutoff, $\xi$ is the corresponding steepness parameter (by default $\xi=\infty$, meaning a sharp transition between $S=1$ at distances smaller than $r_\text{cut}$ to $S=0$ at larger distances, but one can make it more gradual), $r_0$ is the radius of the power-law cutoff, and $\gamma$ is the corresponding power-law index (if negative, this means a suppression at small rather than large radii). In addition, an arbitrary user-defined Python function can be used instead of an instance of \ttt{SelectionFunction} class. The built-in class\\
\texttt{sf = agama.SelectionFunction(point, rcut, xi, r0, gamma)}\\
is mathematically equivalent to\\
\texttt{def sf(x):\\
\mbox{}~~~~dist2 = numpy.sum((x-point)**2, axis=1)\\
\mbox{}~~~~return (numpy.exp(-(dist2 / rcut**2)**(xi/2)) /\\
\mbox{}~~~~~~~~(1 + (dist2 / r0**2)**(gamma/2)))}\\
...
creates the component with a spheroidal DF and an initial guess for the density profile (the latter is not needed if the entire SCM has an initial guess for the potential). The extra \texttt{params} (case-insensitive) depend on the \texttt{disklike}'ness of the component and specify the size and extent of spatial grid for recomputing its density.\\[1mm]
...
\texttt{result = agama.orbit(potential=pot, ic=ic, time=int_time, ...)}\\
here \ppp{ic} contains initial conditions for one orbit (1d array of length 6) or several orbits (a 2d array of shape $N\times6$), total integration \ppp{time} for each orbit may be different, but typically is a multiple of the dynamical time returned by the \texttt{Tcirc} method of \ttt{Potential} (e.g., \texttt{100*pot.Tcirc(ic)}), and dots indicate additional parameters (at least some must be provided to produce a result; all parameter names are case-insensitive):\\
...
\ppp{trajsize}, \ppp{dtype} and \ppp{separateTime} together control the format in which the trajectory is represented, as detailed below.\\
...
\ppp{accuracy} (default $10^{-8}$) is the relative accuracy of the ODE integrator, roughly corresponding to the relative energy error per orbital period. It is usually pretty conservative, and one can speed up the integration by a factor of a few by providing a less tight tolerance ($10^{-4}$ to $10^{-6}$) while still retaining sufficient accuracy.\\
...
The two arrays, \texttt{timestamps} and \texttt{trajectories}, are returned as separate elements of the \texttt{result} tuple when \ppp{separateTime}\texttt{=True}, otherwise combined into a length-2 array \texttt{timetraj} array for a single orbit, or a 2d array of shape $N\times 2$ containing arrays of timestamps in the 0th column and trajectories in the 1st column. For a single orbit, this makes little difference; however, it becomes more important when integrating $N>1$ orbits. If \ppp{separateTime}\texttt{=True} and all orbits have the same \ppp{trajsize}${}=M\ge$1, the routine will automatically combine all timestamps into a single 2d array of shape $N\times M$, and all trajectories -- into another 3d array of shape $N\times M \times \{6\mathrm{\,or\,}3\}$, making the storage very efficient. If orbit sizes differ, this optimization is not possible, so timestamps will be a length-$N$ array containing 1d arrays of length $M_k$, and trajectories will be a length-$N$ array containing 2d arrays of shape $M_k \times \{6\mathrm{\,or\,}3\}$. In both cases, one can access the $j$'th timestamp of $k$'th orbit as \texttt{timestamps[k][j]}, and the $d$'th phase-space coordinate of the corresponding point as \texttt{trajectories[k][j,d]}, but a more direct indexing \texttt{[k,j]} and \texttt{[k,j,d]} is applicable only in the case of optimized storage, when all \texttt{trajsize}'s are the same. However, when \ppp{separateTime}\texttt{=False}, the output array is always heterogeneous (contains 1d arrays in the 0th column and 2d arrays in the 1st column), making such optimization impossible; the indexing scheme becomes \texttt{timetraj[k,0][j]} for timestamps and \texttt{timetraj[k,1][j,d]} for trajectories. This scheme was the only available option in earlier versions of the code, and for compatibility reasons, remains the default setting, but it is now recommended to always explicitly specify \ppp{separateTime}\texttt{=True}, enabling optimized storage whenever possible. In a future version, it will become the default, and eventually the only option.\\
Although the orbit integration is always performed in double precision (\Cpp \texttt{double}, equivalent to \Python \texttt{float}), by default the output format is single precision (\texttt{numpy.float32}) to save memory (with 7 digits of precision usually being enough); setting \ppp{dtype}\texttt{=float} produces double-precision output.\\[1mm]
In the second case, selected by \ppp{dtype}\ttt{=object}, the output trajectory is provided in the form of an interpolator -- an instance of a special class \ttt{agama.Orbit}, which can only be returned by the \ttt{orbit} routine, and cannot be created manually. This class is little more than a thin wrapper for three quintic spline interpolators (implemented by the \ttt{Spline} class) for each Cartesian component of position and velocity as functions of time. Although it can be constructed from points regularly spaced in time (if \ppp{trajsize}$\ge$1), it makes more sense to use the points recorded at every timestep of the ODE integrator (\ppp{trajsize}=0), and this is the default setting that does not need to be specified (unlike the array output format), i.e., it is sufficient to set \ppp{dtype}\texttt{=object}. The \ttt{orbit} routine produces one interpolator object per orbit, i.e., a 1d array of length $N>1$ or a single object in the case of one orbit. When used as a sequence indexed by the \texttt{[]} operator, this object provides the timestamps at which the orbit was recorded, and when used as a function of one variable (time -- a single number or an array), it provides the interpolated trajectory at the given time(s) with a typical accuracy $\sim10^{-6}$ for position and $\sim10^{-5}$ for velocity, which is usually sufficient. This dual interface makes possible a rather amusing syntax, when the actually recorded trajectory is retrieved by applying an \ttt{Orbit} object to itself.\\[1mm]
Examples of usage (assuming that \texttt{posvel} is an array of $N>1$ initial conditions):\\[1mm]
\texttt{result1 = agama.orbit(potential=pot, ic=posvel, time=int_time,\\
\mbox{}~~~~trajsize=1000)\ \ \ }\textit{\color{Sepia}\# old style, not recommended}\\
\texttt{result1_time, result1_traj = agama.orbit(potential=pot, ic=posvel, \\
\mbox{}~~~~time=int_time, trajsize=1000, separateTime=True)\ \ \ }\textit{\color{Sepia}\# new style}\\
...
\texttt{for times,trj in zip(result1_time,result1_traj): plt.plot(times, trj[:,2])}\\
...
\texttt{for trj in result1_traj:\\
\mbox{}~~~~plt.plot((trj[:,0]**2 + trj[:,1]**2)**0.5, trj[:,2])}\\[2mm]
When the computation of deviation vectors is requested by the argument \ppp{der}\texttt{=True}, they are provided in the same format as the trajectory itself. There are six deviation vectors per orbit, so if \ppp{dtype}\texttt{=object}, they are stored as six \ttt{Orbit} objects, otherwise as six arrays of shape $M_k\times\{3\mathrm{\,or\,}6\}$. In the latter case, if all \ppp{trajsize}'s are the same, all vectors for all orbits are combined into a single array of shape $N\times 6 \times M \times\{3\mathrm{\,or\,}6\}$.
A matrix formed by column-stacking the deviation vectors at a given point on the orbit represents the Jacobian $\mathsf{J}(t)$ of mapping between the initial conditions $\boldsymbol w^{(0)}$ and the current point $\boldsymbol w(t)$: $J_{ij} = \partial w_i(t)/\partial w_j^{(0)}$:\\[1mm]
\texttt{delta = numpy.random.normal(size=6)*1e-8\ \ }\textit{\color{Sepia}\# small offset in initial conditions}\\
\textit{\color{Sepia}\# original orbit and deviation vectors}\\
\texttt{time, traj0, der = agama.orbit(potential=pot, ic=posvel[0], time=int_time,\\
\mbox{}~~~~trajsize=trajsize, dtype=float, der=True, separateTime=True)}\\
\textit{\color{Sepia}\# slightly perturbed orbit}\\
\texttt{time, traj1 = agama.orbit(potential=pot, ic=posvel[0]+delta, time=int_time,\\
\mbox{}~~~~trajsize=trajsize, dtype=float, separateTime=True)}\\
\texttt{jac = numpy.dstack(der)\ \ }\textit{\color{Sepia}\# shape: (trajsize,6,6); jac[j] is the Jacobian matrix at time[j]}\\
\texttt{linear_dif = numpy.einsum('ijk,k->ij', jac, delta)\ \ }\textit{\color{Sepia}\# same as jac.dot(delta)}\\
\texttt{actual_dif = traj1-traj0}\\
...
\paragraph{sampleOrbitLibrary} routine constructs an $N$-body snapshot from the orbit library of a Schwarzschild model, in which each orbit has a weight assigned by the \ttt{solveOpt} routine (see Section~\ref{sec:SchwarzschildDetails} for details). For this task, the trajectories need to be stored in the form of \texttt{agama.Orbit} interpolators, so that any number of sampling points can be drawn from each orbit:\\[1mm]
\texttt{
...
nbody = 1000000\\
xv, m = agama.sampleOrbitLibrary(nbody, traj, weights)}\\[2mm]
It produces two arrays: \texttt{nbody}$\times6$ positions/velocities and \texttt{nbody} masses of particles in the $N$-body snapshot (masses are the same for all particles by construction, since the number of points sampled from each orbit is proportional to its weight in the model). If an optional argument \texttt{returnIndices} is set to \texttt{True}, it returns a third array with the indices of orbits from which particles were drawn.
...
A separate mechanism for reading or writing snapshots in the \Nemo format is implemented in the pure-\Python class \texttt{agama.NemoFile}; unlike the other two routines, it can handle files with multiple snapshots. When opened for reading, the instance of this class acts as an iterator retrieving snapshots one after another and returning them as dictionaries containing the following items (none of them are guaranteed to be present): \texttt{Time} (float), \texttt{Position} (2d array of shape $N\times3$), \texttt{Velocity} (same), \texttt{Mass} (1d array of size $N$), and whatever other arrays may be present in the file. When opened for writing or appending, the \texttt{write} method accepts a dictionary with these elements and appends it to the end of the file. The example below performs a trivial copy from input to output file:\\[1mm]
\texttt{
...
%%%%%%%%%%%
\subsection{\textsc{PeTar} plugin}

\textsc{PeTar} \cite{Wang2020} is a modern \Nbody simulation code for collisional stellar systems, for which \Agama can also provide an external potential, e.g., for modelling the evolution of tidally disrupting star clusters in the Milky Way \cite{He2026}. See the folder \texttt{agama-interface} in the experimental branch of \textsc{PeTar} for details and usage examples.
...
\textbf{example_self_consistent_model3} and \textbf{example_self_consistent_model_bdh} perform the same task for a three-component (bulge+disk+halo) galactic model fully specified by DFs, create an $N$-body realization of the model, and plot various physical quantities (density and velocity dispersion profiles, LOSVDs, etc.);\\
...
\textbf{example_mw_nsd} and \textbf{example_mw_nsc} construct DF-based self-consistent model of the Milky Way nuclear stellar disk \cite{Sormani2022a} and nuclear star cluster \cite{Vasiliev2026}.\\
...
This tridiagonal system results in a quintic spline, which provides 5th degree piecewise-polynomial interpolation with three continuous derivatives. It is \textit{not} equivalent to a 5th degree B-spline -- the latter would have 4 continuous derivatives and is fully specified by $K+4$ coefficients, whereas a quintic spline is specified by $2K+2$ numbers (the values and first derivatives at all nodes, plus two endpoint conditions). The accuracy of approximation with a quintic spline is far better than anything achievable with a cubic interpolation -- but only in the case when one may compute the function derivatives independently and accurately enough (it is also possible to construct a quintic spline from function values only, but it will be similar to a cubic spline in terms of accuracy, though of course smoother).
...
\begin{align}
\Phi(r,\theta,\phi) &= \sum_{\ell=0}^{\ell_\mathrm{max}}\sum_{m=-m_0(\ell)}^{m_0(\ell)}
\Phi_{\ell,m}(r)\: \sqrt{4\pi} \tilde P_\ell^m(\cos\theta)\:\trig m\phi, \\
...
\end{array}\right.   \nonumber
\end{align}
Here $\tilde P_\ell^m(x) \equiv \sqrt{\frac{2\ell+1}{4\pi}\frac{(\ell-|m|)!}{(\ell+|m|)!}} \;P_\ell^{|m|}(x)$ 
are normalized associated Legendre polynomials, $\ell_\mathrm{max}$ is the order of expansion in meridional angle $\theta$, and $m_0(\ell) = \mathrm{min}(\ell, m_\mathrm{max})$, where $m_\mathrm{max} \le \ell_\mathrm{max}$ is the order of expansion in azimuthal angle $\phi$ (they do not need to coincide, e.g., if the model is considerably flattened but only weakly triaxial, then $m_\mathrm{max}=2$ may be sufficient, while $\ell_\mathrm{max}$ may be set to 8 or 10). The normalization is chosen so that for a spherically-symmetric potential, $\Phi_{0,0}(r)=\Phi(r)$, and that for each $\ell$, the sum of squared coefficients over all $m$ is invariant under rotations of coordinate system.

In the \ttt{Multipole} class, individual terms are approximated as suitably scaled functions in log-scaled radius. The logarithmic transformation of radius is intended to attain high dynamic range with a moderate number (a few dozen) of grid points, while the transformation of amplitude of each term increases the accuracy of interpolation. These amplitude transformations are used only when the $\ell=0$ harmonic of the potential is everywhere negative (which is usually the case). The main $\ell=0$ term is log-scaled (i.e., the actual function to be interpolated is $\ln\big[1/\Phi(0)-1/\Phi_{0,0}(\ln r)\big]$, where $\Phi(0)$ is the value of potential at origin, which may be finite or $-\infty$). The other terms are normalized to the value of the $\ell=0$ term (i.e., the spline is constructed for $[\Phi_{\ell,m}/\Phi_{0,0}](\ln r)$). Each term is interpolated as a quintic spline, defined by the values and first derivatives at nodes of a radial grid; usually this grid would be linear in log-radius, with a constant ratio between consecutive radii $f\equiv r_{k+1}/r_k$.
...
\begin{align}
\Phi_{\ell,m}(r) &= -\frac{4\pi\,G}{2\ell+1} \left[ r^{-\ell-1} \int_0^r \rho_{\ell,m}(r')\,{r'}^{\,\ell+2}\,\d r' + r^\ell\int_r^\infty \rho_{\ell,m}(r')\,{r'}^{\,1-\ell}\,\d r' \right],  \label{eq:SphHarmPoisson} \\
\rho_{\ell,m}(r) &\equiv \frac{1}{\sqrt{4\pi}} \int_{-1}^1 \d \cos\theta\, \tilde P_\ell^m(\cos\theta) \int_0^{2\pi}\d \phi\:\trig m\phi\:\rho(r,\theta,\phi) .  \label{eq:SphHarmDensity}
\end{align}
...
\bibitem{Binney2026}
Binney J., Wright T., Vasiliev E., 2026, MNRAS, 550, 852
...
\bibitem{He2026}
He Z., Wang L., Zhou Z., Huang Y., Vasiliev E., arXiv:2607.17040
...
\bibitem{Vasiliev2026}
Vasiliev E., Feldmeier-Krause A., Sormani M., 2026, ApJ, 1002, 71
